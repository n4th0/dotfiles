...
async function handle_current_weather() {
    ...
    const response = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=38.34&longitude=-0.48&current_weather=true`);
    const data = await response.json();

    const temperature = data.current_weather.temperature;
    const description = "took 41ms";

    current_weather_element.textContent = `${temperature} ºC`;
};
